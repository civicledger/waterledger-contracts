...
Add a sell order with a price and a quantity in a given zone. Zone details can be retrieved from the `Level0Resources` contract.
...
The Level0Resources contract stores balances for each licence, and the details (such as minimum and maximum balances) of each zone. The pattern for deployment of this differs from most contracts, as the contract is deployed and then each zone in the Scheme needs to be added.
...
```javascript
const level0Resources = ["Barron Zone A", "Barron Zone B", "Barron Zone C", "Barron Zone D", "Barron Zone E"];
...
Level0Resources.forEach(async level0ResourceName => {
  await Level0ResourcesInstance.addZone(web3.utils.toHex(level0ResourceName), 100000, 0, 100000);
});
